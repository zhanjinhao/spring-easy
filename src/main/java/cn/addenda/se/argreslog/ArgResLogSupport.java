...
//                "org.springframework.aop.framework.CglibAopProxy$DynamicAdvisedInterceptor",
        ...
        try {
            ...
            if (argResLogAttr.isLogResult()) {
                if (ArgResLogAttr.ERROR.equals(argResLogAttr.getExceptionLevel())) {
                    ...
                    throw new SystemException("异常时的日志级别只能是：ERROR、WARN、INFO，当前是：" + argResLogAttr.getExceptionLevel() + "。");
                }
            }
    ...
    /**
     * invoke在三种情况下会发生异常：<p/>
     * 第一种情况：ArgRes的使用不正确，此时捕获到的是SystemException，这类异常直接扔出去。此时不走catch块。<br/>
     ...
     * <br/>
     * 第三种情况：ArgRes功能本身出现的异常，这类异常直接扔出去。理论上不会发生。此时不走catch块。<br/>
     */
    protected static void report(ArgResLogException argResLogException) {
